      ...
      <div class="solver-body-contents">
        ...
        <div class="search-col">
          <select class="form-select" @change="selectSort($event)">
            ...
            <option v-for="(item, index) in sortOptions" :key="index" :value="item.value">{{
              item.text
            }}</option>
          </select>
        </div>
        <div class="search-col">
          <select class="form-select" @change="setSubCategory($event)">
            ...
            <option v-for="(item, index) in Category" :key="index" :value="item.code">{{
              item.codeName
            }}</option>
          </select>
        </div>
      ...
      <div class="solver-body font-break" v-for="(solvers, idx1) in solverList" :key="idx1">
        <div class="solver-body-contents">
          <div class="solver-list">
            <div
              class="solver-user-card"
              v-for="(solverData, idx2) in solvers"
              :key="idx2"
              @click="goUserProfile(solverData.nickname)"
            >
              <img
                class="user-level-badge"
                src="@/assets/berry-1.png"
                v-if="parseInt(solverData.point) < 100"
                alt=""
              />
              <img
                class="user-level-badge"
                src="@/assets/berry-2.png"
                v-if="parseInt(solverData.point) >= 100 && parseInt(solverData.point) < 200"
                alt=""
              />
              <img
                class="user-level-badge"
                src="@/assets/berry-3.png"
                v-if="parseInt(solverData.point) >= 200 && parseInt(solverData.point) < 300"
                alt=""
              />
              <img
                class="user-level-badge"
                src="@/assets/berry-4.png"
                v-if="parseInt(solverData.point) >= 300 && parseInt(solverData.point) < 400"
                alt=""
              />
              <img
                class="user-level-badge"
                src="@/assets/berry-5.png"
                v-if="parseInt(solverData.point) >= 400"
                alt=""
              />
              ...
              <div class="box-for-category">
                <span
                  ...
                  v-for="(field, idx3) in solverData.favoriteFieldNameList"
                  :key="idx3"
                  >{{ field }}</span
                >
              </div>
              <div class="solver-user-row">
                ...
                {{
                  isNaN(solverData.evaluationScore)
                    ? 0
                    : parseFloat(solverData.evaluationScore).toFixed(2)
                }}
                <span>/10</span>
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      sortOptions: [
        { text: "팔로워 순", value: "followerDesc" },
        { text: "베리 포인트 순", value: "pointDesc" },
        { text: "베리 점수 순", value: "evaluationDesc" },
      ],
      Category: [],
      subCategory: [],
      // 검색 Params
