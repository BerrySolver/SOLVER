  ...
  <div id="container">
    <div id="wrapper">
      ...
      <p>
        ...
        <button id="stop" @click="clickStop()">Stop Sharing</button>
...
//   let blobs;
//   let blob; // 데이터
//   var rec; // 스트림을 기반으로 동작하는 mediarecorder 객체
//   let stream; // 통합
...
//     stream = new MediaStream(desktopStream);

//     blobs = [];
//     // mimeType: "video/webm; codecs=vp9,opus"

//     rec = new MediaRecorder(desktopStream, { mimeType: "video/webm; codecs=vp9,opus" }); // mediaRecorder객체 생성
...
export default {
  ...
  methods: {
    ...
    clickStop() {
      // this.recoder.stop();
      console.log(this.chuks);
      const blob = new Blob(this.chunks, { mimeType: "video/mp4;codecs=vp8" });
      // console.log(blob);
      // this.blob = blob;
      // this.recordVideo();
    },
  ...
  mounted() {
    ...
    startBtn.onclick = async () => {
      ...
      this.recoder = new MediaRecorder(desktopStream, {
        mimeType: "video/webm; codecs=vp9,opus",
      }); // mediathis.recoderorder객체 생성
      this.recoder.ondataavailable = (e) => blobs.push(e.data);
    ...
    stopBtn.onclick = () => {
      ...
      this.recoder.stop(); // 화면녹화 종료 및 녹화된 영상 다운로드
