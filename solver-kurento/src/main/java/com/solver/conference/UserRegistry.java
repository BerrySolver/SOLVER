...
public class UserRegistry {
	...
	public void register(UserSession user) {
		...
		if (!usersBySessionId.containsKey(user.getSession().getId()))
			usersBySessionId.put(user.getSession().getId(), user);
		else
			usersBySessionId.put(user.getSession().getId(), user);

		System.out.println(usersByName.size());
		System.out.println(usersBySessionId.size() + " " + user);
	}
	
	public void registerName(UserSession user) {
		usersByName.put(user.getName(), user);
	}

	public void reRegister(UserSession user) {
		usersBySessionId.put(user.getSession().getId(), user);
	}
	...
	public void removeScreen(UserSession user) {
		usersByName.remove(user.getName());
	}
