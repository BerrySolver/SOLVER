...
//		final UserSession user = registry.getByName(jsonMessage.get("name").getAsString());
		...
		case "receiveVideoFrom":
			...
			// screen은 registry에 등록이 안돼서 여기서 null이 생기는 듯?
		...
		case "share":
			...
			break;
		case "stopShare":
			stopShare(jsonMessage, user, session);
			break;
		default:
	...
	private void shareScreen(JsonObject params, WebSocketSession session) throws IOException {
		...
		log.info("Screen {}: trying to join room {}", name, roomName);
